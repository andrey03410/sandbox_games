...
from src.games.state import TicTacToeState


class TicTacToe(BaseGame[TicTacToeState]):
    code = "tic_tac_toe"

    def init_state(self, config: dict, player_ids: list[int]) -> TicTacToeState:
    ...
    def apply_move(
        self, state: TicTacToeState, user_id: int, move: dict
    ) -> TicTacToeState:
        row = int(move["row"])
        col = int(move["col"])
    ...
    @staticmethod
    def _in_bounds(state: TicTacToeState, r: int, c: int) -> bool:
        return 0 <= r < state.height and 0 <= c < state.width

    @classmethod
    def _check_win(
        cls, state: TicTacToeState, row: int, col: int, symbol: str
    ) -> list[list[int]] | None:
